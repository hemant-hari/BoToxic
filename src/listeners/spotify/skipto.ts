...
export default {
    ...
    execute: async function (reaction: MessageReaction, user: User, api: SpotifyWebApi) {
        autoRefresh(
            api,
            async () => skipToTrack(reaction, api),
            user,
            reaction.message.channel);
    }
}

async function skipToTrack(reaction: MessageReaction, api: SpotifyWebApi) {
    ...
    return (
        WebApiRequest.builder(accessToken)
            ...
            .withBodyParameters({
                uris: [getURI(reaction.message)],
            })
            .build()
            .execute(HttpManager.put, null)
    );
}